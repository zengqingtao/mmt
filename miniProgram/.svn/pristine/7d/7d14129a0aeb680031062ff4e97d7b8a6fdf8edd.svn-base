...
Page({
      ...
      if (res.statusCode == 200) {
        ...
        common.ApiGateWayTest(promInfoUri, data, true, function (res) {
          ...
          if (Object.prototype.toString.call(promInfoData) === '[object Object]') {
            ...
            var activity_time_start_month = (new Date(promInfoData.start_time * 1000).getMonth() + 1) < 10 ? '0' + (new Date(promInfoData.start_time * 1000).getMonth() + 1) : new Date(promInfoData.start_time * 1000).getMonth() + 1
            var activity_time_start_date = new Date(promInfoData.start_time * 1000).getDate() < 10 ? "0" + new Date(promInfoData.start_time * 1000).getDate() : new Date(promInfoData.start_time * 1000).getDate();
            var activity_time_start_hours = new Date(promInfoData.start_time * 1000).getHours() < 10 ? "0" + new Date(promInfoData.start_time * 1000).getHours() : new Date(promInfoData.start_time * 1000).getHours();
            var activity_time_start_min = new Date(promInfoData.start_time * 1000).getMinutes() < 10 ? "0" + new Date(promInfoData.start_time * 1000).getMinutes() : new Date(promInfoData.start_time * 1000).getMinutes();
  ...
  addShopCart: function(e) {
    ...
    if (type == 'addcart' && that.confirmData[0].sku_key) {
      var invurl = '';
      that.isnewpersongoods(product.goods_id, that.confirmData[0].sku_key).then(function (res) {
        if (res) {
      ...
      function _addcart(){
        ...
        common.formIdUpdate(e);
        if (that.data.stopCartBtn == false) {
          that.data.stopCartBtn = true;
          common.ApiGateWayTest(invurl, invdata, true, function (ret) {
            ...
            if (ret.data.success != 0) {
              ...
              that.data.showSKUstatus = false;
              that.data.stopCartBtn = false;
            } else {
              common.toast(ret.data.msg);
              return;
            }
      ...
      if (type != 'addcart' || !that.data.showSKUstatus) {
        that.setData({
          showSKUstatus: true,
          showfwModalStatus: false,
          showShareModalStatus: false,
    ...
    if (res.target && res.target.dataset.shareinfo) {
      ...
      if (shareInfo && shareInfo.fromButton == 'notice') {
        ...
        return {
          ...
          path: '/pages/product/product?productId=' + that.data.itemData.goods_id + '&pickup_id=' + pickup.pickup_id + '&user_id=' + that.data.itemData.user_id,
  ...
  aldminishare: function(e) {
    var that = this;
    var pickup = common.getStorageSync('getpickup');
    var url = 'pages/product/product?productId=' + that.data.itemData.goods_id + '&entryType=external&pickup_id=' + pickup.pickup_id;
    var data = {};
  ...
  activeChangeMe(e){
    ...
    let data = {
      notice_activity_id: that.data.itemData.notice_activity_id
    }
    common.ApiGateWayTest(url, data, true, function (res) {
      var resData = res.data;
      if (resData.success == 1) {
        if (resData.result.status == 1){
          ...
          that.setData({
            check_remind_activity: !that.data.check_remind_activity
          })
          return
