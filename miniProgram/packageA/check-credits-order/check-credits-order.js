...
Page({
  ...
  onShow: function () {
    ...
    Promise.all([this.getaddresslist()]).then(function (val) {
  ...
  getPreData: function () {
    ...
    for (var i in productInfo) {
      order_product.push({
        score_goods_id: productInfo[i].score_goods_id,
  ...
  getcheckOrderinfo: function () {
    ...
    var uri = app.getPath.checkScoreOrderInfo;
        ...
        if (res.statusCode == 200) {
          ...
          for (var i in res.order_info.firstGoods) {
            app.aldstat.sendEvent("点击下单", {
              "商品名称": res.order_info.firstGoods[i].score_goods_name
            })
          }
        }
        var btn = true;
  ...
  touchSubmit: function (e) {
    ...
    if (that.data.consignee && !that.data.consignee.address_id) {
      ...
      let checkuri=app.getPath.scoreConfirmOrder
      let uri=app.getPath.doPayPoint
      common.ApiGateWayTest(checkuri,data,true,function (res) {
        if(res.data.success==1){
          // common.toast('商品兑换成功')
          common.hideLoad(that);
          wx.redirectTo({
            url: '/packageA/credits-payment/credits-payment?order_id=' + res.data.result.order_id,
          })
        }else{
          common.hideLoad(that);
          that.setData({
